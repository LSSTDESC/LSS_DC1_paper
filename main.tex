...
This paper is structured as follows: In \secref{inputs} we describe the inputs for our simulated images. In \secref{image_generation_pipeline} we describe the process to generate our LSST-like artificial images. Finally, in \secref{conclusions} we present some concluding remarks.
...
which have a magnitude difference smaller than 0.02 magnitudes. We will discuss matching strategies deeper in \secref{matching}.
...
to the input model given the observing conditions and compute the residual.\textcolor{red}{Include the PSF results!!!!}
...
In the case of harmonic space we use the mode projection from \texttt{NaMaster} to correct for the different templates.\CHECK{Add figure with corrected and uncorrected power-spectra.}
