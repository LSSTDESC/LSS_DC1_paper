...
data handling by itself becomes challenging even when considering a small subset of the LSST area.  
...
LSST~\citep{Overview} after 10 years of operation in $r$-band using state of the art tools produced by DESC and LSST, however, these data cover a small fraction (40 deg$^{2}$) of the total LSST area. We characterize the products of this process, validating them, and checking their performance against the requirements set by LSST and DESC in their respective Science Requirements Documents~\citep{LPM-17, 2018arXiv180901669T}. These data products encompass single-visit and coadded calibrated exposures (i.e., flattened, background subtracted, etc.) and source catalogs that take up to $\sim 225$ TB of disk space. We perform the 2-point clustering analysis in harmonic space for these simulations and assess the impact of different potential systematic effects and how dithering can mitigate these effects.
...
\caption{DC1 workflow diagram \textcolor{red}{Get rid of phosim??}.}
...
In order to estimate the depth in the coadd catalogs we generate a flat-sky map, with resolution of $\approx 2$ arcminutes, containing the detected primary sources in the \textit{clean sample} from the previous subsection. This resolution ensures us to have an accurate estimate of the power-spectra up to $\ell \sim 6000$, where it will be mostly dominated by shot-noise. Then, for each pixel, we compute the median SNR of the objects in that pixel as a function of the magnitude and use the magnitude at which SNR is closest to 5.
...
\item Sky-background: We use the observed background level in each exposure and assign that value to the pixels in the flat-sky map that lie within that exposure. After this we calculate the mean value in each pixel to build the map with the same resolution as the mask ($\approx 2$ arcmin) which we deproject~\citep{2019MNRAS.484.4127A}. 
...
\item Number of visits: We count the number of exposures overlapping with each pixel in our maps.
\end{itemize}
These maps are shown in \figref{systematic_maps} and \figref{systematic_maps_ud}. 
...
\caption{Maps showing the different foregrounds considered in our analysis the dithered field. From top left to bottom right: Mean FWHM, mean sky brightness, mean sky noise, mean extinction, stellar density and number of visits in each pixel in the flat-sky maps with the same resolution as the depth maps in \figref{depth_maps}. We only show their values in the regions where the 5-$\sigma$ $r$-band depth is larger than 25.5.}
...
\caption{Same as \figref{systematic_maps} for the undithered dataset. From top left to bottom right we show mean FWHM, mean sky brightness, mean sky noise and number of visits. Note that we use the same extinction and stellar density maps changing the geometry of the mask.}
\label{fig:systematic_maps_ud}
\end{figure*}

We use the mode deprojection from \texttt{NaMaster}~\citep{2019MNRAS.484.4127A} to correct for the potential contamination of the maps described above. In addition, we compute the theoretical prediction for the power-spectra with \texttt{CCL}:
\begin{equation}
