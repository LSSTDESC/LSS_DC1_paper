...
the parameters given by CatSim; and AGNs that are also modeled as point sources and, for simplicity, their variability has been omitted. Future versions of imSim will have the ability to generate more complex morphologies for these sources. The brightness for these sources is computed using the magnitudes from CatSim and converting them to counts using the latest versio for the LSST throughputs\footnote{\url{https://github.com/lsst/throughputs}}. We clip the objects at magnitude 10 to simulate saturation and improve the numerical performance.
...
An example coadd image is shown at \figref{coadd_example}.
...
However, we find that the distributions are asymmetric and that the median is not zero. This effect is even more noticeable in \figref{astrometry_b}, where we accumulated the results for 50 randomly selected visits of the imSim dithered simulation. This effect is also present in the undithered imSim run. We also checked the mean astrometric residual as a function of magnitude in \figref{astrometry_b}. We notice a mild bias for the brightest objects. This bias arises from the fact that the objects in the images include proper motion, which is unaccounted for in our \textit{truth tables}. However, it is below 15 mas, which is a much smaller scale than the resolution of the input N-body simulation. As a consequence we do not expect that the two point clustering statistics will be affected by this bias. 
...
These two procedures yield very similar results (within $\sim 4\%$). We show the relative difference in \figref{depth_comparison}. We checked that the maps built selecting galaxies instead of stars are compatible as well. We will select our footprint according to the depth map using the second methodology. These maps are shown in \figref{depth_maps}.
...
In this section, we analyze the two point clustering statistics for both the dithered and undithered catalogs in real and harmonic space and check the consistency between the input and measured observables. When comparing the input and output catalogs, some subtleties arise. For example, if we select a galaxy sample in the input catalog with $m_{low} < m_{true} < m_{high}$, and you select a second galaxy sample in the output catalog with $m_{low} < m_{measured} < m_{high}$, you get that the power in the second sample is lower than in the first sample. This is due to the fact that the second sample includes galaxies that go fainter.
